...
def plot_network(net, size_distr, cat_list, arxiv_cat_lookup, palette="Accent"):
    ...
    patches = [
        mpatches.Patch(facecolor=c, label=arxiv_cat_lookup[l], edgecolor="black")
        for l, c in color_lookup.items()
    ]
    ...
    # Draw the legend
    ax.legend(
        handles=patches, facecolor="white", loc="lower center", title="Category", ncol=4
    )
...
def plot_comp_network(
    nets, size_distrs, cat_list, arxiv_cat_lookup, topic_category_map, palette="Accent"
):
...
def show_network(
    ...
    nx.draw_networkx_edges(
        new_net_2, pos, width=edge_w, edge_color=ec, ax=ax, alpha=alpha
    )
...
def network_distance_stats(net, name):
    ...
    return pd.Series(
        [n_comps, av_sh_p_l, diam],
        name=name,
        index=["components", "average_path_length", "diameter"],
    )
